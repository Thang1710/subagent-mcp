...
`runtime_check` distinguishes `not_installed`, `auth_required`, `needs_canary`, `ready`, and `incompatible`. When the CLI is missing it returns `INSTALL_REQUIRED`, the searched paths and platform, a current official installation-document link/copyable command, and `next_action=recheck`; it does not download or install. The CLI/UI may open the official installation page or copy the command, but version 1 never executes a Claude installer. When `claude auth status` exits unauthenticated it returns `AUTH_REQUIRED` and advertises the explicit `runtime_authenticate` action. Only after user confirmation may that action launch the resolved standalone CLI with exact argv `claude auth login`, which owns the operating-system default-browser flow. It accepts no path, account, credential, or extra-argument input; a store-backed single-flight lease prevents concurrent UI/MCP launches. The tool returns `auth_pending`, while a later read-only `runtime_check` is the sole authority for successful login and normal `needs_canary`/`ready` recovery.
...
| `runtime_authenticate` | prompt | Open one user-approved native-harness sign-in flow in the OS default browser |
...
`runtime_check`, `runtime_authenticate`, and `runtime_canary` are separate so approval cannot depend on a hidden boolean argument. `auth_pending` is a tool-result state, not a runtime state; `runtime_check` continues to report `auth_required` until the native CLI itself confirms login.
...
Version 1 exposes runtime enablement, executable discovery, variants, context and semantic-permission policies, project trust, health/capability results, update state, and quota-circuit state. It also exposes a read-only current/recent external-agent activity list using the section 11.5 descriptor; this is status visibility, not a chat/operator console. When a runtime explicitly reports `auth_required`, a Sign in button may call the same approval-gated `runtime_authenticate` service action; the native harness, not browser JavaScript, opens the operating-system default browser. The context settings show separate editable `auto_compaction_trigger_tokens` and provider `auto_compaction_window_tokens` fields with the selected adapter's declared ranges; the default trigger target is `274000`, and values outside the ranges are rejected. Unsupported exact-trigger policies are visibly unavailable rather than translated to a provider window. It does not expose agent prompts, transcripts, send/interrupt/close controls, raw events, arbitrary file browsing, or credential fields.
...
The server listens only on explicit loopback addresses, rejects non-loopback peers and unexpected `Host`/`Origin` headers, sends no CORS allowlist, sets a restrictive CSP, and keeps browser sessions and CSRF tokens in memory only. An exact same-origin empty POST may create an `HttpOnly`, `SameSite=Strict` session cookie, so the stable localhost URL remains directly usable; configuration writes and native-auth launches require that cookie plus its per-session CSRF token. The optional per-process bootstrap secret remains available for CLI-open compatibility, is carried in the initial URL fragment rather than query/path, is exchanged once through `X-Subagent-MCP-Token`, removed from browser history, and never placed in logs or referrers. A supplied invalid or replayed token is rejected rather than falling back to automatic session creation. The server fails rather than fall back to a public interface. Managed background stop uses a separate random control token in a bounded product-owned Local file, requires the exact loopback Host and Origin, shuts down cooperatively, and removes only its byte-identical control record. There is no tray application, automatic startup entry, telemetry, or remote-access mode in version 1.
