...
| Local UI attack | Literal loopback bind, one-time fragment bootstrap for each new browser profile, HttpOnly strict cookie, CSRF, Host/Origin checks, restrictive CSP, and no CORS; a tokenless same-origin POST may only restore an existing valid session; background control endpoints additionally require a random token from the product-owned Local control record |
